...
public class Controller {
    ...
    private static final String[] FOOD_TABLE_FIELD_TYPE = { "INTEGER PRIMARY KEY", "TEXT", "TEXT", "TEXT", "REAL", "INTEGER" };
    ...
    private static final String TRANSPORTATION_TABLE_NAME = "transportation";
       private static final String[] TRANSPORTATION_FIELD_NAME = { "_id", "avg_car_price", "avg_gas_price", "avg_diesel_price", "avg_inssurance_price", "unit" ,"avg_monthly_pass_price", "country_code" };
        private static final String[]  TRANSPORTATION_FIELD_TYPE =
            { "INTEGER PRIMARY KEY", "REAL", "REAL", "REAL", "REAL", "TEXT", "REAL","INTEGER" };
    ...
    private static void initDBModelObjects() throws SQLException {
        ...
        // Vietnam
        controller.mVietnamFood = new DBModel(DB_NAME, FOOD_TABLE_NAME, FOOD_TABLE_FIELD_NAME, FOOD_TABLE_FIELD_TYPE);
        controller.mVietnamTransportation = new DBModel(DB_NAME, TRANSPORTATION_TABLE_NAME, TRANSPORTATION_FIELD_NAME,
                TRANSPORTATION_FIELD_TYPE);
        controller.mVietnamRealEstate = new DBModel(DB_NAME, REAL_ESTATE_TABLE_NAME, REAL_ESTATE_FIELD_NAME,
                REAL_ESTATE_FIELD_TYPE);
    }
    ...
    private static void BriazilFoodTransportaionsHouding() throws SQLException
    {
        ...
        for (ArrayList<String> values : brazilo)
        {
             ...
             unit = values.get(3);
             price = Double.valueOf(values.get(4));
            controller.mAllTransportationList.add(new Transportation(description,unit,price,Types.Transportation,country));
        }

        // Housing
        brazilo = controller.mBrazilRealEstate.getAllRecords();
        double buyHouse=0;
        for (ArrayList<String> values : brazilo)
        {

             description = values.get(1);
             buyHouse = Double.valueOf(values.get(2));
             price = Double.valueOf(values.get(3));
            controller.mAllHousingList.add(new Housing(description,buyHouse,price,country));
        }
    }
    private static void addBrazilToLists() throws SQLException
    {
        controller.initializeBrazilDBFromFile();
        BriazilFoodTransportaionsHouding();
    }

    private static void addVietnamTOLists() throws SQLException
    {
        initializeVietnamDBFromFile();
        VietnamFoodTransportaionsHouding();
    }

    private static void VietnamFoodTransportaionsHouding() throws SQLException
    {
        int country = Integer.valueOf(VIETNAM_COUNTRY_CODE);
        //Food
        ArrayList<ArrayList<String>> viet = controller.mVietnamFood.getAllRecords();
        String description,unit;
        double price;
        for (ArrayList<String> values : viet)
        {
        ...
        viet = controller.mVietnamTransportation.getAllRecords();
        for (ArrayList<String> values : viet)
        {
            //TODO Vietnam Transportaions need to be fixed CSV fille not enough!!!
             description = values.get(2);
             unit = values.get(3);
             price = Double.valueOf(values.get(4));
             Transportation x = new Transportation(description,unit,price,Types.Transportation,country);
            controller.mAllTransportationList.add(x);
        }
        // Housing
        viet = controller.mVietnamRealEstate.getAllRecords();
        double buyHouse = 0;
        for (ArrayList<String> values : viet)
        {
    ...
    private int initializeBrazilDBFromFile() throws SQLException {
        ...
        try {
            ...
            while (fileScanner.hasNextLine())
            {
                ...
                controller.mBrazilFood.createRecord(Arrays.copyOfRange(FOOD_TABLE_FIELD_NAME, 1, FOOD_TABLE_FIELD_NAME.length), values);
                recordsCreated++;
            }
            // Transprotaion
            ...
            while (fileScanner.hasNextLine())
            {
                ...
                controller.mBrazilTransportation.createRecord(Arrays.copyOfRange(TRANSPORTATION_FIELD_NAME, 1, TRANSPORTATION_FIELD_NAME.length), values);
            }
    ...
    private static int initializeVietnamDBFromFile() throws SQLException {
        int recordsCreated = 0;
        // FOOD
        if (controller.mVietnamFood.getRecordCount() > 0)
            return 0;
        try {

            Scanner fileScanner = new Scanner(new File("DairyViet.csv"));
            ...
            while (fileScanner.hasNextLine())
            {
                ...
                values[2] = data[2];
                values[3] = data[3];
                values[4] = VIETNAM_COUNTRY_CODE;
                controller.mVietnamFood.createRecord(Arrays.copyOfRange(FOOD_TABLE_FIELD_NAME, 1, FOOD_TABLE_FIELD_NAME.length), values);
                recordsCreated++;
            }
            // Transprotaion
             fileScanner = new Scanner(new File("TransViet.csv"));
            ...
            while (fileScanner.hasNextLine())
            {
                ...
                values[4] = data[0];
                values[5] = data[1];
                values[6] = VIETNAM_COUNTRY_CODE;
                controller.mVietnamTransportation.createRecord(Arrays.copyOfRange(TRANSPORTATION_FIELD_NAME, 1, TRANSPORTATION_FIELD_NAME.length), values);
            }
            // Housing
             fileScanner = new Scanner(new File("TransViet.csv"));
            ...
            while (fileScanner.hasNextLine())
            {
                ...
                values[5] = data[1];
                values[6] = VIETNAM_COUNTRY_CODE;
                controller.mVietnamRealEstate.createRecord(Arrays.copyOfRange(REAL_ESTATE_FIELD_NAME, 1, REAL_ESTATE_FIELD_NAME.length), values);
            }
